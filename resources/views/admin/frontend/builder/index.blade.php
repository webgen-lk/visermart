...
@section('panel')
    @php
        $collections = $pData->page_name == 'home' ? App\Models\ProductCollection::get() : collect([]);
        $promoBanners = $pData->page_name == 'home' ? App\Models\PromotionalBanner::get() : collect([]);
        $activeOffers = $pData->page_name == 'home' ? App\Models\Offer::get() : collect([]);
    @endphp
    <div class="row g-1 g-lg-3">
        @if ($pData->page_name == 'home')
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">@lang('Banner Section Layout')</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <div class="col-12 col-sm-6">
                                    <div class="card shadow-none border">
                                        <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
                                            <h6 class="card-title mb-0"> {{ __($layout['name']) }} </h6>
                                            @if (gs('homepage_layout') == $layout['key'])
                                                <button type="submit" name="name" value="{{ $layout['key'] }}" class="btn btn--success" disabled>
                                                    @lang('SELECTED')
                                                </button>
                                            @else
                                                <form method="post" action="{{ route('admin.setting.home.layouts.update') }}">
                                                    @csrf
                                                    <button type="submit" name="name" value="{{ $layout['key'] }}" class="btn btn--success w-100">
                                                        @lang('SELECT')
                                                    </button>
                                                </form>
                                            @endif
                                        ...
                                        <div class="card-body p-0 text-center">
                                            <img src="{{ $layout['banner'] }}" alt="template">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            @if ($pData->secs != null)
                                @foreach (json_decode($pData->secs) as $sec)
                                    @php
                                        ...
                                        if (!$name) {
                                            $modelData = getSectionData($sec, true);
                                            $name = $modelData->title ?? @$modelData->name;
                                        }
                                    @endphp
                                    <li class="sortable-item highlight icon-move item">
                                        <i class="sortable-icon"></i>
                                        <span class="d-inline-block me-auto"> {{ __($name) }}</span>
                                        <i class="ms-auto d-inline-block remove-icon remove-icon-color las la-trash"></i>
                ...
                <div class="card-body">
                    <ol id="sections_items" class="simple_with_no_drop vertical">
                        ...
                        @foreach ($collections as $collection)
                            <li class="highlight icon-move clearfix" data-key="{{ $collection->unique_key }}">
                                <i class="sortable-icon"></i>
                                <span class="d-inline-block me-auto"> {{ __($collection->title) }}</span>
                                <i class="ms-auto d-inline-block remove-icon remove-icon-color las la-trash"></i>
                                <div class="float-end d-inline-block manage-content">
                                    <a class="btn cog-btn text-center" href="{{ route('admin.collection.update', $collection->id) }}" target="_blank">
                                        <i class="la la-cog m-0 p-0"></i>
                                    </a>
                                </div>
                            </li>
                        ...
                        @foreach ($promoBanners as $promoBanner)
                            <li class="highlight icon-move clearfix" data-key="{{ $promoBanner->unique_key }}">
                                <i class="sortable-icon"></i>
                                <span class="d-inline-block me-auto">{{ __($promoBanner->title) }}</span>
                                <i class="ms-auto d-inline-block remove-icon remove-icon-color las la-trash"></i>
                                <div class="float-end d-inline-block manage-content">
                                    <a class="btn cog-btn text-center" href="{{ route('admin.promo.banner.update', $promoBanner->id) }}" target="_blank">
                                        <i class="la la-cog m-0 p-0"></i>
                                    </a>
                                </div>
                            </li>
                        ...
                        @foreach ($activeOffers as $activeOffer)
                            <li class="highlight icon-move clearfix" data-key="{{ $activeOffer->unique_key }}">
                                <i class="sortable-icon"></i>
                                <span class="d-inline-block me-auto"> {{ __($activeOffer->name) }}</span>
                                <i class="ms-auto d-inline-block remove-icon remove-icon-color las la-trash"></i>
                                <div class="float-end d-inline-block manage-content">
                                    <a class="btn cog-btn text-center" href="{{ route('admin.offer.edit', $activeOffer->id) }}" target="_blank">
                                        <i class="la la-cog m-0 p-0"></i>
                                    </a>
                                </div>
                            </li>
...
@push('script')
    <script>
        (function($) {
            ...
            $('#page_sections').disableSelection();
